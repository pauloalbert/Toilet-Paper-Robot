...
public class TimeMotionProfile {
	 ...
	 * @param maxVelocity ABSOLUTE(>0) the maximum velocity the robot can drive [METERS/SEC] - ABSOLUTE
	...
	public TimeMotionProfile(double finalDistance, double maxVelocity, double acceleration, double deceleration){
		...
		double[] p1 = {0, 0};
		if(Math.abs(Xa) + Math.abs(Xd) >= Math.abs(finalDistance)){ // Triangle Motion Profile
